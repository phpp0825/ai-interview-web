...
class AudioRecorder:
    ...
    def __init__(self, config: Optional[AudioConfig] = None):
        ...
        self.recognizer.energy_threshold = 300  # 기본값 조정
        self.recognizer.dynamic_energy_threshold = True  # 동적 임계값 사용
        ...
        try:
            ...
            with self.microphone as src:
                console.print("[bold yellow]마이크 테스트 중...[/bold yellow]")
                # 짧은 테스트로 마이크 동작 확인
                test_audio = self.recognizer.listen(src, timeout=1, phrase_time_limit=0.5)
                console.print("[bold green]마이크 테스트 완료[/bold green]")
        except sr.WaitTimeoutError:
            console.print("[bold green]마이크 준비 완료 (무음 상태)[/bold green]")
        except Exception as e:
            console.print(f"[bold red]마이크 초기화 경고: {e}[/bold red]")
            # 기본 마이크로 폴백
            self.microphone = sr.Microphone()
        
        self.buffer = []
        self.recording = False
        self.vad = webrtcvad.Vad(2)  # 중간 민감도로 설정 (0:덜 민감 ~ 3:매우 민감)
        self.adjust_noise()
    ...
    def adjust_noise(self):
        ...
        with self.microphone as src:
            ...
            try:
                ...
                self.recognizer.energy_threshold = max(original_threshold, 200)  # 최소값 보장
                self.recognizer.energy_threshold = min(self.recognizer.energy_threshold, 800)  # 최대값 제한
                
                console.print(f"[bold green]에너지 임계값 설정: {self.recognizer.energy_threshold}[/bold green]")
            except Exception as e:
                console.print(f"[bold yellow]소음 조정 실패, 기본값 사용: {e}[/bold yellow]")
